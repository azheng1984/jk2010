<?php
/* @var $this Hyperframework\Web\ViewKernel */
$this->setLayout('_layouts/main.html.php');
$this->setBlock('content', function() {
    ?>
<h2>简单、专业的 PHP 框架。</h2>
<h2>模块</h2>
<h3>Web</h3>
<div>Web 应用开发框架</div>
<h3>Cli</h3>
<div>命令行应用开发框架</div>
<h3>Db</h3>
<div>数据库访问模块</div>
<h3>Logging</h3>
<div>日志模块</div>
<h3>Common</h3>
<div>平台无关的支持模块</div>
<h2>特性</h2>
<h3>简单</h3>
使用简单，并提供手册和 api 文档。
<h3>专业</h3>
轻量级、可扩展的架构，统一的面向对象编码风格，完整的单元测试覆盖。
<h3>模块化</h3>
框架由可单独使用的模块组成，并基于 Composer，无缝整合社区生态系统。
<h3>MVC</h3>
Web 应用架构符合 MVC 模式。
<h3>RESTful</h3>
支持 RESTful 的网站和 api。
<h3>支持过滤器</h3>
控制器支持前置、后置和环绕过滤器。
<h3>优化的错误页面</h3>
通过内部和外部代码分离，错误信息和输出数据分离，辅助开发者更快定位错误。
<h3>支持视图模板</h3>
提供基于原生 PHP 的视图模板系统，并支持整合其他视图模板系统。
<h3>规范化的命令行交互模式</h3>
命令行应用符合 GNU 命令行规范。
<h3>支持 Active Record</h3>
支持 Active Record 模式，用于构建领域模型。
<h3>支持数据库操作剖析</h3>
支持记录数据库操作，用于排错和性能调优。
<h3>支持多数据库应用</h3>
支持同时连接多个数据库，并提供完善的连接管理功能。
<h3>自动化事务处理</h3>
提供了事务的自动提交和回滚功能。
<h3>可扩展的日志系统</h3>
内置的日志系统高度可扩展，并支持整合其他日志系统。
<h3>支持环境配置</h3>
支持为不同的部署环境定制配置。

<br />
<?php });?>
