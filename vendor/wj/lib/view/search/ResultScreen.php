...
class ResultScreen {
  ...
  private static function renderList() {
    ...
    foreach ($GLOBALS['URI']['RESULTS']['matches'] as $id => $content) {
      ...
      echo '<li><p class="image"><a href="http://www.360buy.com/" target="_blank" rel="nofollow"><img alt="'.$name.'" src="http://img.dev.huobiwanjia.com/'.$product['id'].'.jpg" /></a></p><h3><a href="http://www.360buy.com/'.$product['id'].'?source=huobiwanjia.com" target="_blank" rel="nofollow">',
        $title, '</a></h3><p class="price">&yen;<span>',
        $product['lowest_price_x_100']/100,
        '</span></p><p class="description">',
        $description.'&hellip;</p><p class="merchant">京东商城</p></li>';
    }
    echo '</ol>';
  }
}